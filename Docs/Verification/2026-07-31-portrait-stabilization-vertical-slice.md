...
- **Provenance:** final evidence HEAD `d474e55`; raw Unity XML is refreshed at `Logs/playmode-results.xml` by the commands below (the working artifact is intentionally not source-controlled).
...
| Focused capture-session policy | `Test-Unity.ps1 -Platform editmode -Filter JoseonHunter.Tests.EditMode.PortraitStateValidationCapturePolicyTests`; 2 total, 2 passed, 0 failed, 0 skipped, 0 inconclusive; XML start/end `2026-07-31 20:22:49Z`, duration 0.0890554 s. | Passed | Locks the same-process resume rule after a stale cross-process `EditorPrefs` marker triggered `Camera.Render` under `-nographics`. The marker now stores its originating OS PID; a different/missing PID clears every capture key without resuming scene, Play Mode, or rendering work. |
| Full EditMode | `Test-Unity.ps1 -Platform editmode -Filter JoseonHunter.Tests.EditMode`; 529 total, 529 passed, 0 failed, 0 skipped, 0 inconclusive; XML start `2026-07-31 20:24:27Z`, end `2026-07-31 20:24:47Z`, duration 19.974525 s. | Passed | Fresh retained `Logs/editmode-results.xml`; no first-party compile errors found in `Logs/editmode.log`. |
| Full PlayMode | `Test-Unity.ps1 -Platform playmode -Filter JoseonHunter.Tests.PlayMode`; 261 total, 182 passed, 79 failed, 0 skipped, 0 inconclusive; XML start `2026-07-31 20:25:23Z`, end `2026-07-31 20:26:38Z`, duration 74.8044672 s; runner wall time 101.4 s. | Failed | 77 failures are `WeaponPotentialCombatAPlayModeTests` (38) and `WeaponPotentialCombatBPlayModeTests` (39); the other two are `EvolvedWeaponCombatPlayModeTests.Moon_eclipse_keeps_outbound_and_return_contact_then_blasts_at_crossing` and `WeaponAffixVerticalSlicePlayModeTests.Perfect_hwando_jackpot_flows_from_pointer_choice_to_evolution_and_run_reset` (now its Venom Fang committed-mask assertion). The branch-introduced stale explicit-confirmation lifecycle regression was fixed: its earlier `Skip()`-then-`LastCompletedResult` NRE no longer occurs, and focused execution reaches the later mask assertion. These 79 failures remain unresolved release limitations; there is no retained pre-branch full PlayMode baseline, so their origin is not asserted as introduced or pre-existing. No branch mask-topology or executor evidence was found. |
...
| Full automated validation is green | Failed | EditMode green; PlayMode retains the 77 potential-mask consumer failures, Moon Eclipse failure, and the vertical-slice Venom Fang committed-mask failure. |
