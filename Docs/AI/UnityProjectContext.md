...
## Infinite battlefield, authored waves, and Bootstrap loading handoff (2026-08-02)

- Gameplay owns a deterministic 3-by-3 grid of reusable 32-by-32 world-unit battlefield chunks. Exactly nine chunks remain active; crossing a chunk coordinate recycles only changed views, and movement inside one coordinate creates no chunk GameObjects.
- The active ground presentation is the opaque, point-filtered Joseon folk-field tile under `Art/World/Runtime/Battlefield`. The legacy occult field remains available as a fallback. Actual portrait captures at 720x1280 and 1080x2340 showed no black map edge or visible chunk gap.
- Normal enemies now resolve through explicit content IDs. Wave one is plague rats only; wave two adds vengeful spirits; wave three adds sakkat specters; the peak mixes specters, dokkaebi, and bandits. Continuous and pack spawns use one active-combatant budget with a hard mobile ceiling of 140, and new renderers begin outside the real camera viewport.
