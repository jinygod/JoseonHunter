    ...
    public sealed class WeaponPotentialCombatAPlayModeTests
    {
        ...
        public void Armor_break_is_primary_only_and_applies_exact_20_percent_for_two_seconds()
        {
            foreach (var evolved in new[] { false, true })
            {
                ...
                Assert.That(remaining, Is.GreaterThan(.05f));
                rig.AdvanceStatusesOnly(remaining - .05f);
                Assert.That(rig.Runtime.AffixStatuses.HasVulnerabilityForTests(rig.Target.RuntimeId), Is.True);
                Assert.That(rig.Runtime.AffixStatuses.VulnerabilityRemainingForTests(rig.Target.RuntimeId), Is.EqualTo(.05f).Within(.001f));
